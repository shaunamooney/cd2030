...
max_year <- max(penta3_cov_adm2$year)

penta3_cov_adm2 <- penta3_cov_adm2 %>% 
...
# Calculate x positions for category labels
cat_positions <- df %>%
  mutate(x = as.numeric(factor(adminlevel_1, levels = adminlevel_1))) %>%
  summarise(x = mean(x), .by = category)
...
max_year <- max(measles1_cov_adm2$year)

measles1_cov_adm2 <- measles1_cov_adm2 %>% 
