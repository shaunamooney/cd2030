...
print_notes <- function(page_id, object_id = NULL, parameters = NULL) {
  ...
  if (nrow(page_notes) > 0) {
    ...
    cat(paste0("**--- Enter notes related to ", page_id, '. ---**\n\n'))
  }
}
```
...
subnational_indicator_coverge <- adjusted_data %>%
...
measles_subnational_coverage <- analyze_coverage(subnational_indicator_coverge,
                         admin_level = 'adminlevel_1',
                         region = 'Nairobi',
                         indicator = 'measles1',
...
## 5. Equity Analysis: Regional Vaccination Coverage inequality using routine health facility data
...
```{r}
max_year <- max(adjusted_data$year, na.rm = TRUE)
