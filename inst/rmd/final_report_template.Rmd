...
map_mapping <- cache$get_map_mapping()
...
print_notes <- function(page_id, object_id = NULL, parameters = NULL) {
  ...
  if (nrow(page_notes) > 0) {
    ...
    page_notes %>% 
      # reframe(note = list(list(title = title, note = list(note))), .by = title) %>% 
      pull(note) %>% 
      walk(~ {
        cat(paste0(,x, '\n\n'))
        # cat(paste0('**', .x$title, '**\n\n'))
        # walk(.x$note, function(x) cat(paste0(x, '\n\n')))
      })
  } else {
...
```{r, results='asis'}
print_notes('adjustment_changes', 'Live Births')
```
...
```{r, results='asis'}
print_notes('adjustment_changes', 'Penta1')
```
...
```{r, results='asis'}
print_notes('adjustment_changes', 'BCG')
```
...
```{r, results='asis'}
print_notes('adjustment_changes', 'Measles 1')
```
...
dropout_penta13_subnational_coverage <- analyze_coverage(subnational_indicator_coverage,
                         admin_level = 'adminlevel_1',
...
plot(dropout_penta13_subnational_coverage) + cd_report_theme()
```
...
```{r}
dropout_penta3mcv1_subnational_coverage <- analyze_coverage(subnational_indicator_coverage,
                         admin_level = 'adminlevel_1',
                         region = selected_admin_level_1,
                         indicator = 'dropout_penta3mcv1',
                         subnational_map = subnational_map,
...
plot(dropout_penta3mcv1_subnational_coverage) + cd_report_theme()
```
...
```{r}
penta3_subnational_coverage <- analyze_coverage(subnational_indicator_coverage,
                         admin_level = 'adminlevel_1',
                         region = selected_admin_level_1,
                         indicator = 'penta3',
                         subnational_map = subnational_map,
...
plot(penta3_subnational_coverage) + cd_report_theme()
```
...
```{r}
measles1_subnational_coverage <- analyze_coverage(subnational_indicator_coverage,
                         admin_level = 'adminlevel_1',
                         region = selected_admin_level_1,
                         indicator = 'measles1',
                         subnational_map = subnational_map,
...
plot(measles1_subnational_coverage) + cd_report_theme()
```
