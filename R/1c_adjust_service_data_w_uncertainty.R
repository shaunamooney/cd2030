...
#'   - A logical column `impute_rr` to flag where reporting rates are to be imputed
    ...
    mutate(
      across(
        ...
        ~ {
          ...
          k_value <- get(paste0(cur_column(), '_k_value_sample'))
    ...
    mutate(
      ...
      across(
        ...
        ~ {
          ...
          ifelse(outlier == 1, truncnorm::rtruncnorm(1, a = 0, mean = robust_max(med), sd = 50), .)
        }
      ),
      ...
      across(
        ...
        ~ {
          ...
          if_else(
            is.na(.) & !is.na(max_med), truncnorm::rtruncnorm(1, mean = max_med, a = 0, sd = 50),
            .
          )
        }
      ),
