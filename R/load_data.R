...
load_excel_data <- function(filename,
  ...
  dt <- map(data, ~ {
    .x %>%
      ...
      rename_with(~ ifelse(. %in% c("District", "District_name"), "district", .), everything()) %>% # Rename "District" or "District_name" to "district"
  ...
  # numeric_vars <- c("Year", "Total_Population", "Population_ under_5years", "Population_under_1year", "Live_births", "Total_births")
  columns_to_drop <- c(
